...
def create_threshold_transfer_plot(
    ...
    plt.axvline(target_human_data.median(), color=PLOT_COLORS[0], linestyle='--', alpha=0.5, label=f'Human median: {target_human_data.median():.3f}')
    plt.axvline(target_ai_data.median(), color=PLOT_COLORS[3], linestyle='--', alpha=0.5, label=f'AI median: {target_ai_data.median():.3f}')
